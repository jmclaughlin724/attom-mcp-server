...
export function createMcpServer() {
    ...
    server.tool("normalize_address", { address: z.string().describe("Full address to normalize (e.g., '123 Main St, Anytown, CA 12345')") }, async ({ address }) => {
        try {
    ...
    }, async ({ address1, address2, useGoogleNormalization }) => {
        try {
            ...
            if (useGoogleNormalization) {
                try {
                    ...
                    console.warn('[Google Places] Address normalization failed, using original address', error instanceof Error ? error.message : String(error));
                }
            }
    ...
    for (const tool of mcpTools) {
        ...
        server.tool(tool.name, zodSchema, async (params) => {
            try {
    ...
    server.resource("property", new ResourceTemplate("property://{address}", { list: undefined }), async (uri, params) => {
 ...
 */
export async function startMcpServer(transportType = "http") {
